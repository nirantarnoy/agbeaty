...
$query .= "SELECT * FROM product_group WHERE status=1 AND";
...
//    $sub_array[] = $status;
    ...
    $sub_array[] = '<button type="button" name="update" onclick="getupdate($(this))" id="'.$row["id"].'" class="btn btn-warning btn-xs update"><span class="glyphicon glyphicon-pencil"></span> Update</button><button type="button" onclick="getdelete($(this))" name="delete" id="'.$row["id"].'" class="btn btn-danger btn-xs delete" data-status="'.$row["status"].'"><span class="fa fa-trash"></span> Delete</button>';
...
{
    $statement = $connect->prepare("SELECT * FROM product_group");
    $statement->execute();
