...
$query .= "SELECT * FROM product_brand WHERE status=1 AND";
...
//    $sub_array[] = $status;
    ...
    $sub_array[] = '<input type="hidden" class="has-group" value="'.$row['has_group'].'"/><button type="button" onclick="getupdate($(this));" name="update" id="'.$row["id"].'" class="btn btn-warning btn-xs update"><span class="glyphicon glyphicon-pencil"></span> Update</button><button type="button" onclick="getdelete($(this));" name="delete" id="'.$row["id"].'" class="btn btn-danger btn-xs delete" data-status="'.$row["status"].'"><span class="fa fa-trash"></span> Delete</button>';
...
{
    $statement = $connect->prepare("SELECT * FROM product_brand");
    $statement->execute();
